...
function RoofView({ planimetria, className = '' }: { readonly planimetria: PlanimetriaPdfDto | null; readonly className?: string }) {
  ...
  if (planimetria.fotoDataUri) {
    const stile = {
      '--focus-x': `${planimetria.focusXPct ?? 50}%`,
      '--focus-y': `${planimetria.focusYPct ?? 50}%`,
    } as CSSProperties
    return <img className={`roof-image ${className}`} src={planimetria.fotoDataUri} style={stile} alt="Vista del tetto con layout fotovoltaico" />
  }
  return (
...
function Copertina({ dati }: { readonly dati: DatiPdfPreventivo }) {
  ...
  return (
    <PageShell dati={dati} numero={1} id="sintesi" cover>
      ...
      <RoofView planimetria={dati.planimetria} className="cover-roof" />
...
function PaginaDettagli({ dati }: { readonly dati: DatiPdfPreventivo }) {
  ...
  return <PageShell dati={dati} numero={2} id="dettagli"><PageTitle eyebrow="PROGETTO PERSONALIZZATO">Dettagli impianto</PageTitle><div className="two-columns intro-columns"><div><h2>Componenti essenziali dell’offerta</h2><p>{det ? <>L’impianto proposto avrà una potenza complessiva di <b>{det.potenzaKwp}</b> e sarà composto da <b>{det.moduli} pannelli{det.wattPicco ? ` da ${det.wattPicco} Wp` : ''}</b>. La produzione annua stimata è <b>{det.produzioneKwh}</b>{det.consumoKwh ? <> a fronte di un consumo di <b>{det.consumoKwh}</b></> : null}.</> : 'I dati tecnici saranno completati dallo studio tetto associato.'}</p><h2>Regime e agevolazioni</h2><p>{det?.regimeRid ?? 'Da definire'}<br />{det?.detrazioneSintesi ?? ''}</p></div><div className="detail-numbers"><div><span>Potenza</span><strong>{det?.potenzaKwp ?? '—'}</strong></div><div><span>Produzione annua</span><strong>{det?.produzioneKwh ?? '—'}</strong></div><div><span>Resa specifica</span><strong>{det?.resaSpecifica ?? '—'}</strong></div></div></div><h2>Producibilità attesa per l’impianto</h2><RoofView planimetria={dati.planimetria} className="detail-roof" /><div className="roof-caption">{dati.planimetria?.legenda ?? 'Studio tetto EcoSolare'}</div></PageShell>
}
...
function PaginaFinanza({ dati }: { readonly dati: DatiPdfPreventivo }) {
  const sim = dati.simulazione
  const eco = dati.condizioniEconomiche
  return <PageShell dati={dati} numero={12} id="report-finanza" design><BoxCliente dati={dati} /><h2 className="design-section-title">Analisi finanziaria dettagliata</h2><div className="finance-kpis large">{sim?.kpiFinanziari.map((kpi) => <div className={`tone-${kpi.tono}`} key={kpi.etichetta}><span>{kpi.etichetta}</span><strong>{kpi.valore}</strong></div>)}</div><div className="finance-split"><div><span>Investimento complessivo</span><strong>{eco?.totaleLordo ?? dati.totaleLordo}</strong><p>IVA inclusa, prima delle agevolazioni selezionate.</p></div><div><span>Beneficio stimato anno 1</span><strong>{eco?.risparmioAnnuo ?? '—'}</strong><p>Risparmio energetico e valorizzazione dell’energia ceduta.</p></div></div><div className="method-box"><h2>{sim?.termico ? 'Contributo del sistema termico' : 'Perché il risultato è sostenibile'}</h2>{sim?.termico ? <dl><div><dt>Gas evitato</dt><dd>{sim.termico.gasEvitatoSmc}</dd></div><div><dt>Risparmio annuo</dt><dd>{sim.termico.risparmioAnnuo}</dd></div><div><dt>{sim.termico.incentivoEtichetta}</dt><dd>{sim.termico.incentivoImporto ?? '—'}</dd></div></dl> : <Elenco voci={['Produzione, autoconsumo e immissione provengono dallo studio energetico associato.', 'Tariffe, inflazione, degrado e orizzonte sono applicati dal motore di simulazione.', 'Il risultato viene ricalcolato quando cambiano configurazione, prezzo o dati del cliente.']} />}</div><p className="fine-print">{sim?.tariffe}</p></PageShell>
}

function PaginaCashflow({ dati }: { readonly dati: DatiPdfPreventivo }) {
  const sim = dati.simulazione
  return <PageShell dati={dati} numero={13} id="report-cashflow" design><BoxCliente dati={dati} /><h2 className="design-section-title">Analisi flusso di cassa</h2><div className="chart-card"><span>Capitale ancora da recuperare e valore cumulato generato</span><GraficoCashflow punti={sim?.cumulato ?? []} /></div><table className="cashflow-table"><thead><tr><th>Anno</th><th>Risparmio FV</th><th>Detrazioni</th><th>Flusso</th></tr></thead><tbody>{sim?.cashflow.slice(0, 10).map((riga) => <tr key={riga.anno}><td>{riga.anno}</td><td>{riga.risparmio}</td><td>{riga.detrazione}</td><td>{riga.flusso}</td></tr>)}</tbody></table><p className="fine-print">Simulazione EcoSolare Design calcolata sullo studio tetto del cliente. Orizzonte modello: {sim?.orizzonteAnni ?? '—'} anni.</p></PageShell>
}

function PaginaMensile({ dati }: { readonly dati: DatiPdfPreventivo }) {
  const sim = dati.simulazione
  return <PageShell dati={dati} numero={14} id="report-mensile" design><BoxCliente dati={dati} /><h2 className="design-section-title">Energia mensile stimata</h2><div className="chart-card monthly"><span>Produzione mensile (kWh) - profilo annuale</span><GraficoMensile valori={sim?.produzioneMensileKwh ?? []} /><p>Produzione annua complessiva stimata: <b>{dati.dettagliImpianto?.produzioneKwh ?? '—'}</b></p></div><h2 className="design-section-title compact">Layout moduli sul tetto</h2><RoofView planimetria={dati.planimetria} className="monthly-roof" /></PageShell>
}
